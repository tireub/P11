...
while loop:
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            if event.key == K_UP:
                mcgiver.move("up", level, needle, tube,
                             ether, sound, coin_sound)
            if event.key == K_DOWN:
                mcgiver.move("down", level, needle, tube,
                             ether, sound, coin_sound)
            if event.key == K_LEFT:
                mcgiver.move("left", level, needle, tube,
                             ether, sound, coin_sound)
            if event.key == K_RIGHT:
                mcgiver.move("right", level, needle, tube,
                             ether, sound, coin_sound)
        # Takes a push on the sound button into account
        if event.type == MOUSEBUTTONDOWN:
            if event.button == 1:
                if event.pos[0] > 900 and event.pos[0] < 985 and \
                                event.pos[1] > 600 and event.pos[1] < 636:
                    sound.value_switch()
                    if sound.state == 1:
                        pygame.mixer.pause()
                    if sound.state == 0:
    ...
    if mcgiver.sprite == [level.finish[1], level.finish[0]]:
        break
